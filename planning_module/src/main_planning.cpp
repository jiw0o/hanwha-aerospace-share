...
//        graph_pub_.publish(graph_markers_);
    ...
    // void visualizeFrontiers(std::vector<std::pair<double, double>>& frontier_positions, double odom_position_x, double odom_position_y, double prev_odom_position_x, double prev_odom_position_y, double odom_rotation_yaw, double prev_odom_rotation_yaw) {
    //     int counter = 0;
    //     // 기존 frontier를 시각화
    //     for (const auto& frontier : frontier_positions) {
    //         visualization_msgs::Marker marker;
    ...
    //     // 오돔 기준으로 변환된 frontier를 시각화
    //     for (const auto& frontier : frontier_positions) {
    //         Point transformed_frontier = this->transformFrontier(frontier, prev_odom_position_x, prev_odom_position_y, prev_odom_rotation_yaw, odom_position_x, odom_position_y, odom_rotation_yaw);

    //         visualization_msgs::Marker transformed_marker;
    //         transformed_marker.header.frame_id = "base_link";
    //         transformed_marker.header.stamp = ros::Time::now();
    //         transformed_marker.ns = "transformed_frontier";
    //         transformed_marker.id = counter++;
    //         transformed_marker.type = visualization_msgs::Marker::SPHERE;
    //         transformed_marker.action = visualization_msgs::Marker::ADD;
    //         transformed_marker.pose.position.x = transformed_frontier.x;
    //         transformed_marker.pose.position.y = transformed_frontier.y;
    //         transformed_marker.pose.position.z = 1.1;
    //         transformed_marker.scale.x = 1.0;
    //         transformed_marker.scale.y = 1.0;
    //         transformed_marker.scale.z = 2.5;
    //         transformed_marker.color.a = 1.0;
    //         transformed_marker.color.r = 1.0;  // 빨간색으로 변환된 frontier 표시
    //         transformed_marker.color.g = 0.0;
    //         transformed_marker.color.b = 0.0;
    //         transformed_marker.lifetime = ros::Duration(marker_lifetime_);
    //         frontier_markers_.markers.push_back(transformed_marker);
    //     }
...
//            aitstar->setRewireFactor(3.0);
    ...
    //     ROS_INFO("Goal cost at projected goal: %.2f", cost_value);

    //     // 프로젝트된 목표가 유효하지 않으면 프론티어에서 가장 가까운 지점을 찾음
    //     // if (std::isnan(cost_value) || cost_value >= SVC_cost_threshold_) {
    //         // ROS_WARN("Invalid projected goal position. Searching for nearest frontier point...");
        
    //     if(!is_first_planning_){
    //         if (!frontier_set_.empty()) {
    //             std::vector<std::pair<double, double>> transformed_frontiers;
    //             double delta_x = odom_position_x_ - prev_odom_position_x_;
    //             double delta_y = odom_position_y_ - prev_odom_position_y_;
    //             double delta_yaw = odom_rotation_yaw_ - prev_odom_rotation_yaw_;
    //             std::cout << "delta_x: " << delta_x << ", delta_y: " << delta_y << ", delta_yaw: " << delta_yaw << std::endl;

    //             for (const auto& frontier : frontier_set_) {
    //                 // 이전 오돔 위치 기준의 프론티어 좌표에서 이전 오돔 위치를 빼서 상대 위치를 얻음
    //                 double rel_x = frontier.first - delta_x;
    //                 double rel_y = frontier.second - delta_y;

    //                 // 회전 변환 적용 (this->rotatePoint 함수 사용)
    //                 Point rotated_point = this->rotatePoint(rel_x, rel_y, -delta_yaw);

    //                 // 변환된 프론티어 위치를 현재 오돔 위치에 더하여 현재 프레임에서의 위치를 얻음
    //                 double transformed_x = rotated_point.x;
    //                 double transformed_y = rotated_point.y;

    //                 transformed_frontiers.push_back({transformed_x, transformed_y});
    //             }

    //             // 변환된 프론티어 중에서 프로젝트된 목표와 가장 가까운 지점 찾기
    //             double min_dist = std::numeric_limits<double>::max();
    //             std::pair<double, double> closest_frontier = transformed_frontiers[0];

    //             for (const auto& frontier : transformed_frontiers) {
    //                 double distance = std::sqrt(std::pow(frontier.first - local_goal_x_, 2) + std::pow(frontier.second - local_goal_y_, 2));
    ...
    //             std::cout << "size of transformed_frontiers: " << transformed_frontiers.size() << std::endl;

    //             // // 가장 가까운 프론티어를 새로운 로컬 목표로 설정
    //             // local_goal_x_ = closest_frontier.first;
    //             // local_goal_y_ = closest_frontier.second;

    //             visualizer_.visualizeFrontiers_local(transformed_frontiers);
    //             ROS_INFO("\033[1;95mNew goal set to closest frontier at: (%.2f, %.2f)", local_goal_x_, local_goal_y_);
    //         } else {
    //             ROS_ERROR("No valid frontiers found. Unable to set goal.");
    //             // 필요한 경우, 추가적인 처리 로직을 구현할 수 있습니다.
    //             // line search 
    //         }
    //     }else{
    ...
    //     }
    //     // } else {
    //     //     ROS_INFO("\033[1;93mProjected goal is valid at: (%.2f, %.2f), cost: %.2f\033[0m", local_goal_x_, local_goal_y_, cost_value);
    //     // }
...
//
//            double cost_value = this->get_cost_value(local_goal_state);
//            ROS_INFO("Goal Search cost: %.2f", cost_value);
//
//            // 목표 지점의 비용이 유효하지 않거나 threshold를 넘으면 frontier에서 가장 가까운 위치로 설정
//            if (std::isnan(cost_value) || cost_value >= SVC_cost_threshold_) {
//                ROS_WARN("Invalid goal position. Searching for nearest frontier point...");
//
//                if (!frontier_set_.empty()) {
//                    double min_dist = std::numeric_limits<double>::max();
//                    std::pair<double, double> closest_frontier = frontier_set_[0];
//
//                    // 현재 오돔 기준으로 frontier 위치를 회전하고 이동을 보상한 후 가장 가까운 frontier 찾기
//                    for (const auto& frontier : frontier_set_) {
//                        // frontier 위치를 prev_odom 기준에서 현재 오돔 기준으로 변환
//                        Point transformed_frontier = this->transformFrontier(frontier, prev_odom_position_x_, prev_odom_position_y_, prev_odom_rotation_yaw_, odom_position_x_, odom_position_y_, odom_rotation_yaw_);
//
//                        // 현재 오돔 기준으로 frontier까지의 거리 계산
//                        double distance = std::sqrt(std::pow(transformed_frontier.x - odom_position_x_, 2) + std::pow(transformed_frontier.y - odom_position_y_, 2));
//                        if (distance < min_dist) {
//                            min_dist = distance;
//                            closest_frontier = {transformed_frontier.x, transformed_frontier.y};
//                        }
//                    }
//
//                    // closest_frontier를 새로운 local goal로 설정
//                    local_goal_x_ = closest_frontier.first;
//                    local_goal_y_ = closest_frontier.second;
//                    ROS_INFO("\033[1;95mNew goal set to closest frontier at: (%.2f, %.2f)", local_goal_x_, local_goal_y_);
//                    is_goal_valid = true;
...
//            // goal이 유효하지 않은 경우 처리
//            if (!is_goal_valid) {
//                ROS_ERROR("No valid goal found. Aborting planning.");
//            }
//        }
//        else{
//            is_first_planning_ = false;
...
//
//            if (!frontier_set_.empty()) {
//                // frontier를 현재 odom에 맞게 변환
//                std::vector<std::pair<double, double>> transformed_frontiers;
//                for (const auto& frontier : frontier_set_) {
//                    Point transformed_frontier = this->transformFrontier(frontier, prev_odom_position_x_, prev_odom_position_y_, prev_odom_rotation_yaw_, odom_position_x_, odom_position_y_, odom_rotation_yaw_);
//                    transformed_frontiers.push_back({transformed_frontier.x, transformed_frontier.y});
//                }
//
//                // 변환된 frontiers를 시각화
//                visualizer_.visualizeFrontiers_local(transformed_frontiers);
...
//                std::pair<double, double> closest_frontier = transformed_frontiers[0];
//
//                // 변환된 frontier 중에서 가장 가까운 frontier 찾기
//                for (const auto& frontier : transformed_frontiers) {
//                    // 현재 오돔 기준으로 frontier까지의 거리 계산
//                    double distance = std::sqrt(std::pow(frontier.first - odom_position_x_, 2) + std::pow(frontier.second - odom_position_y_, 2));
//                    if (distance < min_dist) {
//                        min_dist = distance;
//                        closest_frontier = frontier;
//                    }
