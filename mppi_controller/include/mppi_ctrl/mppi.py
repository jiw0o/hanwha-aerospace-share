...
class MPPI():
    ...
    def command(self, state):
        ...
        indices = torch.where(cost_total == beta)[0]
        if len(indices) != 1:
            min_index = 0
        else:
            min_index = indices.item()
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        state = state.repeat(self.M, 1, 1) # state: 1 x M x 6
        ...
        for t in range(T):
            ...
            prev_state = state
            # start = time.time()
            state, next_pose = self._dynamics(state, u, t) # 1 x M x 6
            # end = time.time()
            # print("Time taken for dynamics: ", end-start)
            self.predicted_traj.append(next_pose)
