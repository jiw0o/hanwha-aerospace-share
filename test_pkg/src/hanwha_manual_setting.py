...
def create_results_file():
    with open(results_file_name, "w", newline='') as csvfile:
        fieldnames = ['test_case_index', 'start_index', 'goal_index', 'distance', 'driving_time', 'average_lin_speed', 'average_ang_speed', 'success']
...
def odometry_callback(msg):
    global current_position, global_goal, current_test_case, lin_velocity_log, ang_velocity_log
    ...
    if global_goal and current_test_case :
        ...
        if distance < threshold:
            ...
            if duration is not None:
                ...
                append_results()  # Append results to CSV after each goal is reached
                rospy.loginfo(fc['yellow']+f"\nResults are saved in {results_file_name}."+fc['off'])
            ...
            if duration is not None:
                ...
                append_results()  # Append results to CSV after timeout
                rospy.loginfo(fc['yellow']+f"\nResults are saved in {results_file_name}."+fc['off'])
            set_next_goal(current_test_case.success)

def set_next_goal(success=False):
    global current_test_case, is_succuess_reset_pose
