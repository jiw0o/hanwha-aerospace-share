...
waypoints = [
    ...
    WayPoint(-164.742020, -151.474289, 0.182272, 0.000031, 0.000002, -1.500553),    #1 (2)
    # WayPoint(-22.505510, -173.799545, 0.182272, 0.000025, -0.000021, -0.720796),    #3
    WayPoint(121.051787, -144.940808, 0.182282, -0.000004, 0.000001, 1.605975),    #2
    # WayPoint(164.758188, -130.812440, 0.182224, -0.000219, -0.000117, 1.621685),    #4
    WayPoint(107.723560, 31.089726, 0.182336, 0.000134, -0.000030, 1.723253),       #3 (5)
    WayPoint(9.231220, 156.007675, 17.163360, -0.293528, 0.044426, 2.774223),       #4 (6)
    WayPoint(-140.764633, 175.916199, 11.174964, -0.016145, 0.268510, -1.269022)    #5 (7)
...
def odometry_callback(msg):
    global current_position, global_goal, current_test_case, done_epoch, lin_velocity_log, ang_velocity_log
    current_position = (msg.pose.pose.position.x, msg.pose.pose.position.y)
    lin_velocity_log.append(abs(msg.twist.twist.linear.x))
    ang_velocity_log.append(abs(msg.twist.twist.angular.z))
    if global_goal and current_test_case and not done_epoch:
        ...
        elif time.time() - current_test_case.start_time > current_test_case.timeout:
...
def set_next_goal(success=False):
    global current_test_case, test_cases, epoch, current_epoch, done_epoch, is_succuess_reset_pose
    pause_gazebo()  # Pause Gazebo simulation on goal reached
    if len(test_cases) == 0:
        done_epoch = True
        rospy.loginfo(fc['yellow']+f"Done epoch {current_epoch}. Save and move to next"+fc['off'])
        stop_odometry_subscription()
        current_epoch += 1
        if current_epoch > epoch:
            rospy.loginfo(fc['yellow']+f"\nDone all of {epoch} epochs. Test completed."+fc['off'])
            return
        test_cases = initial_test_cases.copy()
        set_next_goal(success)
        start_odometry_subscription()
        done_epoch = False
        return
    current_test_case = test_cases.pop(0)  # Pop the first test case and assign it to current_test_case
    distance = np.linalg.norm(
        np.array((waypoints[current_test_case.start_index].x, waypoints[current_test_case.start_index].y)) -
        np.array((waypoints[current_test_case.goal_index].x, waypoints[current_test_case.goal_index].y))
    )
    timeout = distance / limit_velocity * margin
    rospy.loginfo(fc['green']+f"\n==========[Epoch {current_epoch}/{epoch}] Test Case {current_test_case.case_id} =========="+fc['off'])
...
if __name__ == '__main__':
    # Initialize ROS Node
    rospy.init_node('hanwha_loop_test_node')
